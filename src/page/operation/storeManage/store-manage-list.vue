<template>
	<div>
		<div class="common-search m-5">
	    	<input type="text" class="span-10" placeholder="用途" v-model="searchName">
	    ...
	    <div class="iscroll">
				...
				<div v-if="projectList.length > 0">
				    <div class="project-detail row bg-white project-status-failed" v-bind:class="{ ' project-status-success': item.workStautsCode==='5' | item.workStautsCode==='2' }" v-for="(item,index) in projectList" :key="index">
				    	<div class="fl project-text pt-10 pb-10">
				    		<div class="pb-10">
				    			<span class="text-light-grey span-3 inline tr">用途</span>
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.projectName }}</span></span>
				    		</div>
				    		<div>
				    			<span class="text-light-grey span-3 inblock tr vertical-top">申请人姓名</span>
			...
			<returntop :iscroll='storeListIscroll'></returntop>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		updated () {
			//	刷新iscroll
			if (this.storeListIscroll) {
				this.storeListIscroll.refresh();
			}
		},
		mounted () {
			this.initTop('#wrapper', '.iscroll');
			this.storeListIscroll = this.initIscroll('#wrapper');
			this.loadmore(this.storeListIscroll, this.pullRefresh, this.pullMore);
		},
		beforeDestroy () {
			this.storeListIscroll.destroy();
			this.storeListIscroll = null;
		}
	};
</script>
