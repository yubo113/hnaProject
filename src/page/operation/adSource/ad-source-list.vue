<template>
	<div class="csr-work" >
		<div class="common-search m-5">
	    	<input type="text" class="span-10" placeholder="申请标题" v-model="searchName">
	    ...
	    <div class="iscroll">
				...
				<div v-if="projectList.length > 0">
				    <div class="project-detail row bg-white project-status-failed" v-bind:class="{ ' project-status-success': item.workStautsCode==='5' | item.workStautsCode==='2' }" v-for="(item,index) in projectList" :key="index">
				    	<div class="fl project-text pt-10 pb-10">
				    		<div>
				    			<span class="text-light-grey span-3 inline tr">申请标题</span>
				    		...
				    		<div>
				    			<span class="text-light-grey span-3 inline tr">申请人姓名</span>
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.reportRelTime }}</span></span>
				    		</div>
				    		<div>
				    			<span class="text-light-grey span-3 inblock tr vertical-top">申请人状态</span>
			...
			<returntop :iscroll='adlistIscroll'></returntop>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			enterApp: function (people, item) {
				this.$router.push({name: 'adSource'});
			},
		...
		updated () {
			//	刷新iscroll
			if (this.adlistIscroll) {
				this.adlistIscroll.refresh();
			}
		},
		mounted () {
			this.initTop('#wrapper', '.iscroll');
			this.adlistIscroll = this.initIscroll('#wrapper');
			this.loadmore(this.adlistIscroll, this.pullRefresh, this.pullMore);
		},
		beforeDestroy () {
			this.adlistIscroll.destroy();
			this.adlistIscroll = null;
		}
	};
</script>
