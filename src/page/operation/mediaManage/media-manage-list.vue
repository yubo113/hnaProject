<template>
	<div>
	    ...
	    <div class="iscroll">
				...
				<div v-if="projectList.length > 0">
				    <div class="project-detail row bg-white project-status-failed" v-bind:class="{ ' project-status-success': item.workStautsCode==='5' | item.workStautsCode==='2' }" v-for="(item,index) in projectList" :key="index">
				    	<div class="fl project-text pt-10 pb-10">
				    		<div class="pb-10">
				    			<span class="text-light-grey span-3 inline tr">申请公司</span>
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.projectName }}</span></span>
				    		</div>
				    		<div class="pb-10">
				    			<span class="text-light-grey span-3 inline tr">媒体账号</span>
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.projectName }}</span></span>
				    		</div>
				    		<div>
				    			<span class="text-light-grey span-3 inblock tr vertical-top">账户负责人</span>
			...
			<returntop :iscroll='mediaListIscroll'></returntop>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			enterApp: function (people, item) {
				this.$router.push({name: 'mediaManage'});
			},
		...
		updated () {
			//	刷新iscroll
			if (this.mediaListIscroll) {
				this.mediaListIscroll.refresh();
			}
		},
		mounted () {
			this.initTop('#wrapper', '.iscroll');
			this.mediaListIscroll = this.initIscroll('#wrapper');
			this.loadmore(this.mediaListIscroll, this.pullRefresh, this.pullMore);
		},
		beforeDestroy () {
			this.mediaListIscroll.destroy();
			this.mediaListIscroll = null;
		}
	};
</script>
