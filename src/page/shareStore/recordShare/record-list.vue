<template>
	<div class="csr-work" >
		<div class="common-search m-5">
	    	<input type="text" class="span-10" placeholder="项目标题" v-model="searchName">
				    ...
				    <div class="project-detail row bg-white charitable-status" v-for="(item,index) in projectList" :key="index">
				    	<div class="fl project-text pt-10 pb-10" @click="enterApp(item.id)">
				    		<div>
				    			...
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.title }}</span></span>
				    		</div>
				    		<div class="pt-10 pb-10">
				    			<span class="text-light-grey span-3 inline tr">项目类型</span>
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.recordType }}</span></span>
				    		</div>
				    	</div>
...
<script>
	...
	export default {
		...
		created () {
			...
			this.$store.commit('setShareIndex', 1);
			this.getPartnerList();
		},
		watch: {
		},
		methods: {
			...
			enterApp: function (id) {
				this.$router.push({name: 'recordForm', query: {id: id}});
			},
			getPartnerList: function (name) {
				const self = this;
				return this.$post({
					url: '/app/mainReq?reqUrl=/mobile/hnabResProject/listRecord',
					params: {
						title: name,
						pageSize: 10,
