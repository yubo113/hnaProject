				    ...
				    <div class="project-detail row bg-white project-status-failed" v-bind:class="{ ' project-status-success': item.workStautsCode==='5' | item.workStautsCode==='2' }" v-for="(item,index) in projectList" :key="index">
				    	<div class="fl project-text pt-10 pb-10">
				    		...
				    		<div>
				    			...
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.assigneTime }}</span></span>
				    		</div>
				    		<div>
...
<script>
	...
	export default {
		...
		created () {
			this.$store.commit('changeTitle', '十佳项目评选');
		...
		methods: {
			...
			enterApp: function (people, item) {
				...
				if (people.thisStatus === '2') {
					this.$router.push({name: 'topTenReport', query: people});
				} else {
					this.$router.push({name: 'topTenHisory', query: people});
				}
			},
			getCsrWork: function (name) {
				const self = this;
				return this.$post({
					url: '/app/mainReq?reqUrl=/mobile/topTenSelection/list',
					params: {
						projectName: name,
						pageSize: 6,
