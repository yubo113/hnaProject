<template>
	<div class="csr-work" >
		<div class="common-search m-5">
	    	<input type="text" class="span-10" placeholder="论坛名称" v-model="searchName">
				...
				<div v-if="projectList.length > 0">
				    <div class="project-detail row bg-white project-status-failed" v-bind:class="{ ' project-status-success': item.workStautsCode==='5' | item.workStautsCode==='2' }" v-for="(item,index) in projectList" :key="index">
				    	<div class="fl project-text pt-10 pb-10">
				    		<div>
				    			<span class="text-light-grey span-3 inline tr">论坛名称</span>
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.forumName }}</span></span>
				    		</div>
				    		<div>
				    			<span class="text-light-grey span-3 inline tr">论坛地址</span>
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.forumPlace }}</span></span>
				    		</div>				    		
				    		<div>
				    			<span class="text-light-grey span-3 inline tr">论坛开始时间</span>
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.forumTimeS }}</span></span>
				    		</div>
				    		<div>
				    		...
				    		<div>
				    			...
				    			<span class="span-6 inblock vertical-top">:<span class="pl-10 text-blue inblock" v-for="(people,index) in item.csrForumSonList" :key="index" @click="enterApp(people, item)">{{people.participantName}}</span></span>
				    		</div>
				    	</div>
...
<script>
	...
	export default {
		...
		created () {
			this.$store.commit('changeTitle', 'CSR论坛');
		...
		methods: {
			...
			enterApp: function (people, item) {
				people.forumName = item.forumName;
				if (people.thisStatus === '2') {
					this.$router.push({name: 'csrForumReport', query: people});
				} else {
					this.$router.push({name: 'csrForumHisory', query: people});
				}
			},
			getCsrWork: function (name) {
				const self = this;
				return this.$post({
					url: '/app/mainReq?reqUrl=/mobile/csrForum/list',
					params: {
						forumName: name,
						pageSize: 6,
					...
					if (res.result) {
						console.log(res);
						//	当数据成功并且有数据时候执行
						if (res.data && res.data.csrForumList && res.data.csrForumList.length > 0) {
							//	判断数据是否是上拉加载还是刷新
							if (self.isPullDown) {
								self.projectList = self.projectList.concat(res.data.csrForumList);
								self.isPullDown = false;
							} else {
								self.projectList = res.data.csrForumList;
							}
							this.pageCount += 1;
						} else {
							self.projectList = [];
