			...
			<div id="scroller">
			    ...
			    <div>
			    	<div class="app-report row bg-white pt-20">
			    		<div class="mb-20">
			    			<span class="text-light-grey span-2 inline tr">刊期</span>
			    			<span class="span-7 textOverflow inline app-report-text">{{ $route.query.projectPen }}</span>
			    		</div>
			    		<div class="mb-20">
...
<script>
	...
	import detailFirst from './weeklyObserveComponents/weekly-detail-1.vue';
	import detailSecond from './weeklyObserveComponents/weekly-detail-2.vue';
	export default {
		...
		created () {
			...
			this.$store.commit('changeTitle', '每周责任观察审批');
			this.getReport();
		},
		watch: {
		},
		methods: {
			...
			getReport: function () {
				...
				this.$post({
					url: '/app/mainReq?reqUrl=/mobile/csrWeeklyDuty/approvalDetail',
					params: {
			...
			reportOperate: function (resultObj) {
				this.$post({
					url: '/app/mainReq?reqUrl=/dynamicson/hnabCsrRespDynamicson/approvalResult',
					params: {
						'id': this.$route.query.id,
			...
			enterHisReport: function () {
				if (this.readonly) {
					this.$router.replace({name: 'weeklyHisory', query: this.$route.query});
				} else {
					this.$router.push({name: 'weeklyHisory', query: this.$route.query});
				}
			},
			//	返回点击结果
