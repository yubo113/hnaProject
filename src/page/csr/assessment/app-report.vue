...
<script>
	...
	export default {
		components: { 'detailFirst': detailFirst, 'detailSecond': detailSecond, 'detailThird': detailThird, 'detailFourth': detailFourth, 'detailFifth': detailFifth, 'detailSixth': detailSixth, 'detailSeventh': detailSeventh },
		data () {
			return {
				appReportIscroll: '',
				appReportDetail: {},
		...
		created () {
			...
			this.$store.commit('changeTitle', 'CSR评估审批');
			this.getReport();
		},
		watch: {
		},
		methods: {
			...
			getReport: function () {
				...
				this.$post({
					url: '/app/mainReq?reqUrl=/mobile/csrAssessment/approvalDetail',
					params: {
			...
			reportOperate: function (resultObj) {
				this.$post({
					url: '/app/mainReq?reqUrl=/assessment/hnabCsrAssessmentson/approvalResult',
					params: {
						'id': this.$route.query.id,
			...
			enterHisReport: function () {
				if (this.readonly) {
					this.$router.replace({name: 'assessmentHisory', query: this.$route.query});
				} else {
					this.$router.push({name: 'assessmentHisory', query: this.$route.query});
				}
			},
			//	返回点击结果
