				    ...
				    <div class="project-detail row bg-white project-status-failed" v-bind:class="{ ' project-status-success': item.workStautsCode==='5' | item.workStautsCode==='2' }" v-for="(item,index) in projectList" :key="index">
				    	<div class="fl project-text pt-10 pb-10">
				    		<div>
				    			...
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.proname }}</span></span>
				    		</div>
				    		<div>
				    		...
				    		<div>
				    			...
				    			<span class="span-6 inblock vertical-top">:<span class="pl-10 text-blue inblock" v-for="(people,index) in item.csrAssessmentSonList" :key="index" @click="enterApp(people, item)">{{people.participantName}}</span></span>
				    		</div>
				    	</div>
...
<script>
	...
	export default {
		...
		created () {
			this.$store.commit('changeTitle', 'CSR评估');
		...
		methods: {
			...
			enterApp: function (people, item) {
				people.proname = item.proname;
				if (people.thisStatus === '2') {
					this.$router.push({name: 'assessmentReport', query: people});
				} else {
					this.$router.push({name: 'assessmentHisory', query: people});
				}
			},
			getCsrWork: function (name) {
				const self = this;
				return this.$post({
					url: '/app/mainReq?reqUrl=/mobile/csrAssessment/list',
					params: {
						proname: name,
						pageSize: 6,
					...
					if (res.result) {
						console.log(res);
						//	当数据成功并且有数据时候执行
						if (res.data && res.data.csrAssessmentList && res.data.csrAssessmentList.length > 0) {
							//	判断数据是否是上拉加载还是刷新
							if (self.isPullDown) {
								self.projectList = self.projectList.concat(res.data.csrAssessmentList);
								self.isPullDown = false;
							} else {
								self.projectList = res.data.csrAssessmentList;
							}
							this.pageCount += 1;
						} else {
							self.projectList = [];
