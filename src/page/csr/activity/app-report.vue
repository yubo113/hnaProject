			    ...
			    <div>
			    	<div class="app-report row bg-white pt-20">
			    		<div class="mb-20">
			    			...
			    			<span class="span-7 textOverflow inline app-report-text">{{ $route.query.actionName }}</span>
			    		</div>
			    		<div class="mb-20">
...
<script>
	...
	export default {
		components: { 'detailFirst': detailFirst, 'detailSecond': detailSecond, 'detailThird': detailThird, 'detailFourth': detailFourth, 'detailFifth': detailFifth },
		data () {
			return {
				appReportIscroll: '',
				appReportDetail: {},
		...
		created () {
			...
			this.$store.commit('changeTitle', 'CSR对外活动审批');
			this.getReport();
		},
		watch: {
		},
		methods: {
			...
			getReport: function () {
				...
				this.$post({
					url: '/app/mainReq?reqUrl=/mobile/csrExtActivity/approvalDetail',
					params: {
			...
			reportOperate: function (resultObj) {
				this.$post({
					url: '/app/mainReq?reqUrl=/srcactivi/hnabCsrExternalActivi/approvalResult1',
					params: {
						'id': this.$route.query.id,
			...
			enterHisReport: function () {
				if (this.readonly) {
					this.$router.replace({name: 'activityHisory', query: this.$route.query});
				} else {
					this.$router.push({name: 'activityHisory', query: this.$route.query});
				}
			},
			//	返回点击结果
