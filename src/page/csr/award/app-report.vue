...
<script>
	...
	export default {
		...
		created () {
			...
			this.$store.commit('changeTitle', 'CSR奖项申报审批');
			this.getReport();
		},
		watch: {
		},
		methods: {
			...
			reportOperate: function (resultObj) {
				this.$post({
					url: '/app/mainReq?reqUrl=/srcactivi/hnabCsrExternalActivi/approvalResult',
					params: {
						'id': this.$route.query.id,
			...
			enterHisReport: function () {
				if (this.readonly) {
					this.$router.replace({name: 'awardHisory', query: this.$route.query});
				} else {
					this.$router.push({name: 'awardHisory', query: this.$route.query});
				}
			},
			//	返回点击结果
