<template>
	<div class="csr-work" >
		<div class="common-search m-5">
	    	<input type="text" class="span-10" placeholder="刊期名称" v-model="searchName">
				...
				<div v-if="projectList.length > 0">
				    <div class="project-detail row bg-white project-status-failed" v-bind:class="{ ' project-status-success': item.workStautsCode==='5' | item.workStautsCode==='2' }" v-for="(item,index) in projectList" :key="index">
				    	<div class="fl project-text pt-10 pb-10">
				    		<div>
				    			<span class="text-light-grey span-3 inline tr">期刊名称</span>
				    			<span class="span-6 textOverflow inline">:<span class="pl-10">{{ item.frequencyName }}</span></span>
				    		</div>
				    		<div>
				    		...
				    		<div>
				    			...
				    			<span>:<span class="pl-10">{{ item.workStatus }}</span></span>
				    		</div>
				    		<div>
				    			<span class="text-light-grey span-3 inblock tr vertical-top">参与人</span>
				    			<span class="span-6 inblock vertical-top">:<span class="pl-10 text-blue inblock" v-for="(people,index) in item.csrMonthLySonList" :key="index" @click="enterApp(people, item)">{{people.orgPeopleName}}</span></span>
				    		</div>
				    	</div>
...
<script>
	...
	export default {
		...
		created () {
			this.$store.commit('changeTitle', 'CSR工作专刊');
		...
		methods: {
			...
			enterApp: function (people, item) {
				people.frequencyName = item.frequencyName;
				if (people.thisStatus === '2') {
					this.$router.push({name: 'monthlyReport', query: people});
				} else {
					this.$router.push({name: 'monthlyHisory', query: people});
				}
			},
			getCsrWork: function (name) {
				const self = this;
				return this.$post({
					url: '/app/mainReq?reqUrl=/mobile/csrMonthly/list',
					params: {
						frequencyName: name,
						pageSize: 6,
					...
					if (res.result) {
						console.log(res);
						//	当数据成功并且有数据时候执行
						if (res.data && res.data.csrMonthlyList && res.data.csrMonthlyList.length > 0) {
							//	判断数据是否是上拉加载还是刷新
							if (self.isPullDown) {
								self.projectList = self.projectList.concat(res.data.csrMonthlyList);
								self.isPullDown = false;
							} else {
								self.projectList = res.data.csrMonthlyList;
							}
							this.pageCount += 1;
						} else {
							self.projectList = [];
