				    ...
				    <div class="project-detail row bg-white project-status-failed" v-bind:class="{ ' project-status-success': item.workStautsCode==='5' | item.workStautsCode==='2' }" v-for="(item,index) in projectList" :key="index">
				    	<div class="fl project-text pt-10 pb-10">
				    		...
				    		<div>
				    			...
				    			<span class="span-6 inblock vertical-top">:<span class="pl-10 text-blue inblock" v-for="(people,index) in item.csrOpenDaySonList" :key="index" @click="enterApp(people, item)">{{people.participantname}}</span></span>
				    		</div>
				    	</div>
...
<script>
	...
	export default {
		...
		created () {
			this.$store.commit('changeTitle', '海航开放日');
		...
		methods: {
			...
			enterApp: function (people, item) {
				...
				if (people.thisstatus === '2') {
					this.$router.push({name: 'openDayReport', query: people});
				} else {
					this.$router.push({name: 'openDayHisory', query: people});
				}
			},
			getCsrWork: function (name) {
				const self = this;
				return this.$post({
					url: '/app/mainReq?reqUrl=/mobile/csrOpenDay/list',
					params: {
						proname: name,
						pageSize: 6,
					...
					if (res.result) {
						console.log(res);
						//	当数据成功并且有数据时候执行
						if (res.data && res.data.csrOpenDayList && res.data.csrOpenDayList.length > 0) {
							//	判断数据是否是上拉加载还是刷新
							if (self.isPullDown) {
								self.projectList = self.projectList.concat(res.data.csrOpenDayList);
								self.isPullDown = false;
							} else {
								self.projectList = res.data.csrOpenDayList;
							}
							this.pageCount += 1;
						} else {
							self.projectList = [];
