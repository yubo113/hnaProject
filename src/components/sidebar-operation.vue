<template>
	<div class="layer overflow" v-bind:class="{ 'layerShow': active, 'layerHide': !active }">
		<div class="sider span-8 fl bg-black">
			<div class="siderTitle bg-operation pl-50 page-head">
				<div class="inner-center">
					<img class="siderTitleImg inline" src="static/img/csr/operation.png" @click="enterIndex"/>
					<span class="text-white inline" @click="enterIndex">品牌运营</span>
				</div>
			</div>
			<div class="text-white menuHeight" >
			<div>
				<div class="menuList pl-10" v-for="(group , index) in groupList" :key="index" :class="{'bg-operation': openIndex === index}">
	...
	export default {
		data () {
			return {
				...
				groupList: [{
					name: '广告资源审批',
					router: '/index/operation/adSourceList',
					content: []
				}, {
					name: '库存管理',
					router: '/index/operation/storeManageList',
					content: []
				}, {
					name: '礼品采购',
					router: '/index/operation/giftStockList',
					content: []
				}, {
					name: '新媒体账户管理',
					router: '/index/operation/mediaManageList',
					content: []
				}]
			};
		},
